...
if !GetGlobalBool("VManipEnabled") then return end

local ply=LocalPlayer()
if ply:GetViewEntity() != ply and !self:IsActive() then return end
if IsValid(ply:GetActiveWeapon()) then if ply:GetActiveWeapon():GetHoldType()=="duel" or ply:GetActiveWeapon().CW20Weapon then return false end --doesnt always work
...
VManip.PlayAnimInsidePVS = function(self,anim,plrTable)
	if !GetGlobalBool("VManipEnabled") then return end 
	local traceData = {output = GAMEMODE.traceResultOutput}
	...
	local ourShootPos = lply:GetShootPos()
	local ourAimVec = lply:GetAimVector()

	traceData.start = ourShootPos
	traceData.filter = ply

	local looptable = plrTable or player.GetAll()

	for _,v in ipairs(looptable) do
		        local pos = v:GetBonePosition(v:LookupBone("ValveBiped.Bip01_Head1")) -- stealed from cl_hud cus GMod hasnt way to check player PVS clientside
