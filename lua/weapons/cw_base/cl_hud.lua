...
function SWEP:drawCustomHUD()
	...
	if GetConVarNumber("cw_customhud_ammo") <= 0 then
		...
		if reloadProgress then
			...
			draw.ShadowText(self.HUD_MagText .. self:getMagCapacity(), "CW_HUD32", baseXAMMO + 15, baseYAMMO, self.HUDColors.white, self.HUDColors.black, 1, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
		end -- self.HUD_MagText .. self:getMagCapacity()
