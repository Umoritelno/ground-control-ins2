...
function GM:handleMarkerData(markPos, text, color, displayTime)
	if type(markPos) == "Vector" then
		GAMEMODE:AddMarker(markPos, text, color, displayTime)
	else -- table
		for key, data in ipairs(markPos) do
			GAMEMODE:AddMarker(data, text, color, displayTime)
		end
	end
end
...
GM:registerRadioVoiceVariant("ghetto", "ghetto", nil, 
	{"models/player/group01/male_03.mdl", "models/player/group01/male_01.mdl"}, {"models/player/group01/male_03.mdl", "models/player/group01/male_01.mdl"}, true, true)
...
function command:receive(sender, commandId, category, markPos)
	GAMEMODE:handleMarkerData(markPos, self.onScreenText, self.onScreenColor, self.displayTime)
end
...
function command:send(ply, commandId, category)
	if #ply.loggedKillDatas > 0 then
		local sendData = {}
		local ct = CurTime()
		
		for key, data in ipairs(ply.loggedKillDatas) do
			if ct < data.t then
				table.insert(sendData, data.p)
			end
		end
		
		if #sendData > 0 then
			GAMEMODE:sendMarkedSpot(category, commandId, ply, team.GetPlayers(ply:Team()), sendData)
			...
			if team.GetAlivePlayers(ply:Team()) > 1 and not GAMEMODE.RoundOver then -- only give the reward if there's at least one player alive or the round hasn't ended yet
				ply:addCurrency(self.cashReward * #sendData, self.expReward * #sendData, "REPORT_ENEMY_DEATH")
			end
		end
