...
gcBaseButton.text = ""
gcBaseButton.hoverR, gcBaseButton.hoverG, gcBaseButton.hoverB, gcBaseButton.hoverA = 153, 198, 153, 255
...
function gcWeaponPanel:GetBackgroundColor()
	if GetConVarNumber(self.ConVar) == self.weaponID then
		return 174, 226, 174, 255
	else
		if self:IsHovered() then
			return 153, 198, 153, 255
		end
	end
	
	return 136, 157, 160, 255
end
...
function weightBar:Paint()
	...
	local weightPercentage = math.min(1, curWeight / GAMEMODE.MaxWeight)
...
function attachmentSelection:createInfoBox()
	if not IsValid(self.descBox) then
		...
		if y + newHeight + h > _SCRH then -- if we don't have enough space vertically, we align it to bottom
			y = _SCRH - newHeight
		else
			y = y + h
		end
		...
		cam.IgnoreZ(true)
			...
			elseif not self.validCategory then
				...
				if self.validCategory then
					surface.SetDrawColor(153, 198, 153, 255)
				else
					surface.SetDrawColor(255, 255, 255, 255)
				end
			end
...
function roundOver:Paint()
	...
	local yOff = _S(12)
	draw.ShadowText(self.winningTeamName, GAMEMODE.PopupFont, w * 0.5, yOff, White, Black, 1, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	draw.ShadowText(self:GetFinalBottomText(), GAMEMODE.PopupFont, w * 0.5, h - yOff, White, Black, 1, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
...
function gcArmorDisplay:Paint()
	...
	if self:IsHovered() then
		...
		surface.SetDrawColor(69, 80, 81, 255)
	end
...
function gcGenericPopup:Paint()
	...
	draw.ShadowText(self.topText, self.font, w * 0.5, yOff, White, Black, 1, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	
	if self.bottomText then
		draw.ShadowText(self.bottomText, self.font, w * 0.5, h - yOff, White, Black, 1, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	end
...
function gcMVPDisplay:SetPlayer(ply)
	...
	self.avatarImage:SetSize(h - _S(4), h - _S(4))
	self.avatarImage:SetPos(_S(2), _S(2))
...
function gcMVPDisplay:Paint()
	...
	draw.LinearGradient(h, 0, w - h - _S(1), h - _S(2), self.startColor, self.finishColor, draw.VERTICAL, w)
	
	local fontHeight = draw.GetFontHeight(gcMVPDisplay.mainTextFont)
	
	local four = _S(4)
	draw.ShadowText(self.playerText, self.mainTextFont, h + four, 0, White, Black, 1, TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP)
	draw.ShadowText(self.scoreText, self.scoreTextFont, h + four, fontHeight + _S(2), White, Black, 1, TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP)
end	
...
function gcKillerDisplay:SetKillData(killer, inflictorData)
	...
	self.avatarImage = vgui.Create("AvatarImage", self)
	self.avatarImage:SetSize(h - _S(3), h - _S(3))
	self.avatarImage:SetPos(_S(2), _S(2))
	self.avatarImage:SetPlayer(killer, _S(self.avatarImageSize))
...
function gcKillerDisplay:Paint()
	...
	draw.ShadowText(self.playerText, self.mainTextFont, h + _S(4), _S(1), White, Black, 1, TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP)
	draw.ShadowText(self.killerText, self.scoreTextFont, h + _S(4), fontHeight + _S(2), White, Black, 1, TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP)
	
	if self.teamKill then -- draw a separate element so that people can't fake TK reports as easily
		DisableClipping(true)
			surface.SetDrawColor(0, 0, 0, 230)
			local tkSpace = _S(self.tkSpacing)
			surface.DrawRect(w + tkSpace, 0, h, h)
			draw.ShadowText("TK", self.tkFont, w + tkSpace + h * 0.5, _S(1) + h * 0.5 - draw.GetFontHeight(self.tkFont) * 0.5, White, Black, 1, TEXT_ALIGN_CENTER, TEXT_ALIGN_CETER)
