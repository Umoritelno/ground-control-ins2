...
function GM:prepareArmorPiece(ply, armorId, category)
	local armorCategory = self.Armor[category]
	
	if not armorCategory then
		return
	end
	
	local armorPiece = armorCategory[armorId]
	
	if not armorPiece then
		return
	end
	
	local armorObject = {health = armorPiece.health, id = armorId, category = category}
...
steelHelmet.damageDecreasePenetration = 0.1 -- percentage of damage to negate, in case of penetration, consider not putting this above 20%
steelHelmet.health = 200
steelHelmet.icon = "ground_control/hud/armor/aa_steel_helmet"
steelHelmet.description = "Provides type II protection against projectiles."

GM:registerArmor(steelHelmet)

local kevlarHelmet = {}
kevlarHelmet.category = "helmet"
kevlarHelmet.id = "kevlar_helmet"
...
kevlarHelmet.damageDecreasePenetration = 0.125
kevlarHelmet.health = 150
kevlarHelmet.icon = "ground_control/hud/armor/aa_kevlar_helmet"
kevlarHelmet.description = "Provides type IIIA protection against projectiles."

GM:registerArmor(kevlarHelmet)
