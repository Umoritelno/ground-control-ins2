...
GM.tipController.events = { -- key is event name
	BEGIN_BLEEDING = {times = 6, text = "Being shot in an unprotected area will cause bleeding. Press Q_MENU_KEY to bandage yourself.", formatFunc = function(text) return string.gsub(text, "Q_MENU_KEY", GAMEMODE:getKeyBind("+menu")) end},
	STOPPED_BLEEDING = {times = 6, text = "Applying a bandage will not restore health, be careful around gunfire."},
	PICKUP_WEAPON = {times = 6, text = "Picking up weapons will not transmit the owner's ammo, and you can not carry more than 2 weapons at a time."},
	DROPPED_WEAPON = {times = 10, text = "Taking too much damage to the arms will make you drop your primary and be unable to use them."}, -- for when we lose a lot of health via hits to the arm(s) and we lose our primary
	RADIO_USED = {times = 6, text = "Radios can be used for quick communication and marking enemy positions. Press C_MENU_KEY to open the radio menu.", formatFunc = function(text) return string.gsub(text, "C_MENU_KEY", GAMEMODE:getKeyBind("+menu_context")) end},
	KILLED_ENEMY = {times = 6, text = "Make sure to report enemy deaths using the radio, it marks the death area and gives you a cash and experience bonus."},
	MARK_ENEMIES = {times = 6, text = "Make sure to spot enemies using the radio, it'll give you a spot bonus for enemies killed within the marked area."},
	HEAL_TEAMMATES = {times = 6, text = "You can bandage your team-mates the same way you can bandage yourself."},
	RESUPPLY_TEAMMATES = {times = 6, text = "If you have any spare ammo, give it to team-mates that need some."},
	HELP_TEAMMATES = {times = 6, text = "Make sure to help your team-mates, alone you're rarely a threat to the enemy."},
	HIGH_WEIGHT = {times = 6, text = "Carrying a lot of equipment will cause your stamina to drain faster from sprinting."},
	SPEND_CASH = {times = 6, text = "Spend your earned Cash on weapon attachments and Traits."},
	UNLOCK_ATTACHMENT_SLOTS = {times = 6, text = "Earning Experience will unlock more attachment slots for your guns."},
	HIGH_ADRENALINE = {times = 6, text = "Being suppressed increases your run speed and hip-fire accuracy, but makes aimed fire difficult."},
	FASTER_MOVEMENT = {times = 6, text = "Switching to a lighter weapon allows for faster movement."},
	HEALTH_REGEN = {times = 6, text = "Damage negated by an armor vest without penetration is equal to the amount of health you can regenerate idly."},
...
if CLIENT then
	...
	function GM.tipController:displayEvent(event)
		...
		self.displayTime = CurTime() + (math.Clamp(2 + #text * 0.05, 2, 8)) + _S(2)
		
		surface.SetFont(GAMEMODE.TipFont)
	...
	function GM.tipController:draw(w, h)
		...
		if self.alpha > 0 then
			...
			surface.DrawRect(w * 0.5 - self.displayWidth * 0.5 - four, h * 0.5 + eighty - self.displayHeight * 0.5 - _S(2), self.displayWidth + _S(28), self.displayHeight + four)
		
			local qMarkSize = _S(16)
			
			surface.SetDrawColor(255, 255, 255, 255 * self.alpha)
			surface.SetTexture(questionMark)
			surface.DrawTexturedRect(w * 0.5 - self.displayWidth * 0.5, h * 0.5 + _S(72), qMarkSize, qMarkSize)
			
			local hudColors = GAMEMODE.HUDColors
			
			hudColors.white.a = 255 * self.alpha
			hudColors.black.a = 255 * self.alpha
				draw.ShadowText(self.displayText, GAMEMODE.TipFont, w * 0.5 + _S(20), h * 0.5 + eighty, hudColors.white, hudColors.black, 1, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
