...
PLAYER.CLIMB_STAMINA_DRAIN = 15 -- lose this much stamina per every second we spend climbing
PLAYER.CLIMB_WEIGHT_DRAIN = 0.4 -- per every kilogram we will lose an extra of this much stamina while climbing
...
function PLAYER:isCurClimbValid()
	local pos = self:GetPos()
	--local finish = pos * 1
	--local hullMax = GAMEMODE.StandHullMax
	
	traceData.start = pos
	traceData.endpos = Vector(pos.x, pos.y, pos.z + GAMEMODE.StandHullMax.y)
	traceData.filter = self
	traceData.mask = PLAYER.SOLID_CLIMB_MASK
	traceData.mins = GAMEMODE.StandHullMin
	traceData.maxs = GAMEMODE.StandHullMax
...
function PLAYER:attemptClimb(moveData)
	if not self.climbDestination then
		...
		if self:isCurClimbValid() and self:isClimbAreaUnobstructed(self.climbDestination) then
			if self.climbState == PLAYER.CLIMB_STATE_CLIMB then
				if not self:tooFarApartFromClimbDest() then
					if not self:reachedClimbDestination() then
						...
						local drain = PLAYER.CLIMB_STAMINA_DRAIN + self:calculateWeight() * PLAYER.CLIMB_WEIGHT_DRAIN
						
						self:setStamina(self.stamina - drain * FrameTime())
					else
						self:advanceClimb()
					end
				else
