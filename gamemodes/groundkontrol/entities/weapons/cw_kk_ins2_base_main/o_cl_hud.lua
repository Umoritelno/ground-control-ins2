...
function SWEP:getReserveAmmoText()
	...
	if self.KKINS2RCE then
		return ammoCount, shouldOverride, self.HUDColors.white
	end

	if self.KKINS2Nade then
		return ammoCount, shouldOverride, targetColor
	end
			...
			if self.dt.INS2GLActive then
				...
				if reloadProgress then
					...
					if magCap > 0 or self.KKINS2RCE then
						draw.ShadowText(self:getMagCapacity() .. " / " .. ammoCount, "CW_HUD60", 90, 50, self.HUDColors.white, self.HUDColors.black, 2, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
					else
