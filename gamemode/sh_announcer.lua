		...
		if player then
			...
			--for key, ply in ipairs(players) do
				net.Start("GC_ANNOUNCEMENT")
					...
					net.WriteString(id)
					net.WriteFloat(seed)
				net.Send(team.GetPlayers(targetTeam))
			--end
		end
	else
...
if CLIENT then
	function GM:GetAnnouncementSoundFromCache(announcerId, announcementId, seed)
		...
		if not announcementData.sounds then
			return
		end
...
GM:registerAnnouncement("ghetto", "drugs_retrieved", {
	...
	"ground_control/radio/ghetto/drugs_retrieved4.mp3"})
	
GM:registerAnnouncement("ghetto", "drugs_retrieved_ex", nil, function()
	GAMEMODE.curGametype:verifyDefendersCapturePointText()
end)
...
GM:registerAnnouncement("ghetto", "retrieve_drugs", {
	...
	"ground_control/radio/ghetto/retrieve_drugs3.mp3"}, function()
		GAMEMODE.curGametype:verifyDefendersCapturePointText()
	end)
